...
class ScoringServiceTest {
    ...
    private ScoringService service() {
        ScoringConfig config = new ScoringConfig(
                new ScoringConfig.Weights(0.45, 0.35, 0.20, 0.20, 0.15),
                new ScoringConfig.Thresholds(500, 3, 4, 24, 5.0, 3, 24, 5.0),
    ...
    void singletonMetricGetsMidRank50AndRenormalizesToItself() {
        List<BrandScore> scores = service().compute(
                Map.of(1L, 0.5), Map.of(), Map.of(), Map.of(), Map.of(), AT);
    ...
    void renormalizesWhenSoldoutMissing() {
        // rank: brand2 = 중앙값 → 50, fan: brand2 = 상위 → 75, soldout/snap/review 없음
        List<BrandScore> scores = service().compute(
    ...
    void snapBuzzContributesToTotalAndRenormalizesCorrectly() {
        // brand1: rank=50(유일), snap=75(유일). soldout/fan/review 없음.
    ...
    @Test
    void reviewVelocityContributesToTotalAndRenormalizesCorrectly() {
        // brand1: review=75(상위). rank/fan/soldout/snap 없음.
        // total = (75*0.15) / 0.15 = 75.00 (단독 재정규화)
        List<BrandScore> scores = service().compute(
                Map.of(),
                Map.of(),
                Map.of(),
        ...
        BrandScore s = byBrand(scores, 1L);
        assertThat(s.getReviewVelocityScore()).isEqualByComparingTo("75.00");
        assertThat(s.getRankChangeScore()).isEqualByComparingTo("0.00");
        assertThat(s.getSnapBuzzScore()).isEqualByComparingTo("0.00");
        // review 단독 재정규화: (75*0.15) / 0.15 = 75.00
        assertThat(s.getTotalScore()).isEqualByComparingTo("75.00");
    }
}
